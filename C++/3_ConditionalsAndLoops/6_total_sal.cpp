...
int main(){
 char grade;
 double basic_sal, hra, da, allow, pf, total_sal;
 
 cout << "Enter basic salary and grade (seperated by space):";
 cin >> basic_sal >> grade;

 if (0 <= basic_sal && basic_sal <= 7500000) { 
    ...
    if (grade == 'A')
        ...
        allow = 1300;
    //totalSalary = basic + hra + da + allow – pf
    total_sal = basic_sal + hra + da + allow - pf; 
    int total = round(total_sal);
